    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                // 성운 노이즈
                float2 nebulaUV = uv * _NebulaScale + float2(_Time.y * _NebulaSpeed, _Time.y * _NebulaSpeed * 0.6);
                float nebula = fbm(nebulaUV);
                nebula = pow(nebula, 1.5);
                col.rgb += _NebulaColor.rgb * nebula * _NebulaOpacity;

                // 별
                float star = stars(uv, _StarDensity);
                col.rgb += star * _StarBrightness;

                // 엣지 글로우
                float2 edgeDist = min(uv, 1.0 - uv); // 각 엣지까지 거리
